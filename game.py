...
if player_choose == '1':
    ...
    # 选择2
    print('你竟然选择了白骨精，太不要脸了，你将以->白骨精<-的身份来进行游戏！')
else:
    # 选择3
...
# 显示玩家的信息（攻击力、声明值）
print(f'{player_name},你的生命值是 {player_life} , 你的攻击力是 {player_attack}')

# 由于游戏选项是需要反复显示的，索引必须将其编写到一个循环中
while True:
    # 打印一条分割线
    print('-'*66)
    # 显示游戏选项，游戏正式开始
    ...
    if game_choose == '1':
        ...
        if boss_life <= 0:
            ...
            print(f'->{boss_name}<- 受到了 {player_attack} 点伤害，重伤不治死了，->{player_name}<-赢得了胜利')
            # 游戏结束
            break
        ...
        if player_life <= 0:
            # 玩家死亡
            print(f'你受到了 {boss_attack} 点伤害 ，重伤不治死亡了！GAME OVER')
            # 游戏结束
            break
    elif game_choose == '3':
        # 打印一条分割线
        print('-'*66)
        # 逃跑，退出游戏
        print(f'->{player_name}<- , 撒腿就跑！GAME OVER')
        break
    else:
        # 打印一条分隔线
        print('-'*66)
